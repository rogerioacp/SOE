...
#include "backend/storage/buffer/sgxbufmgr.h"
...
_hash_getinitbuf(VRelation rel, BlockNumber blkno)
{
	...
	/* initialize the page */
	_hash_pageinit(BufferGetPage(vrel, buf), BufferGetPageSize(vrel, buf));
 ...
 */
Buffer
_hash_getnewbuf(VRelation rel, BlockNumber blkno, ForkNumber forkNum)
{
